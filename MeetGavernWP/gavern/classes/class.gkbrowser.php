 ...
 * Browser class
 *
 * Used to detect the user's device
 *
 **/
  ...
  public function __construct() {
	$browser = '';
     ...
     if (isset($_SERVER['HTTP_USER_AGENT'])) {
        ...
        $is_IE6 = preg_match('/msie\s(5\.[5-9]|[6]\.[0-9]*).*(win)/i', $browser) && !preg_match('/msie\s([7-9]\.[0-9]*).*(win)/i', $browser);
        $fb_crawler = preg_match('/facebook/i', $browser);
        $w3c_validator = preg_match('/w3c/i', $browser);
        ...
        if ($fb_crawler) {
            ...
            switch ($this->getBrowser()) {
                case 'mozilla':
                    ...
                    break;
                case 'opera':
                	if(preg_match('/opera\smobi/i', $browser)) {
                ...
                case 'msie':
                    if (preg_match('/iemobile/i', $browser)) {
                    	$this->result->set('mobile', true);
                    	$this->result->set('browser', 'handheld');
                    } else {
	                    if ($is_IE6) {
	                        ...
	                        if (preg_match('/msie\s[7]/i', $browser))
	                            $this->result->set('browser', 'ie7');
	                        else
	                            if (preg_match('/msie\s[8]/i', $browser))
	                                $this->result->set('browser', 'ie8');
	                            else
	                                if (preg_match('/msie\s[9]/i', $browser))
    ...
    /**
     *
     * Function used to return the results of the browser detection
     *
    ...
    public function getBrowser() {
		...
		if (!empty($agent)) {
			if (preg_match('|Opera[/ ]([0-9.]+)|', $agent, $version)) {
				return 'opera';
			} elseif (preg_match('|Chrome[/ ]([0-9.]+)|', $agent, $version)) {
				return 'chrome';
			} elseif ((preg_match('|MSIE ([0-9.]+)|', $agent, $version)) || (preg_match('|Internet Explorer/([0-9.]+)|', $agent, $version))) {
				return 'msie';
			}
			elseif (preg_match('|Konqueror/([0-9]+)|', $agent, $version) || preg_match('|Safari/([0-9]+)\.?([0-9]+)?|', $agent, $version)) {	    	
